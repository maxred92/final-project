...
def send_news_email_task(things_of_last_week, user:dict):
    ...
    for things in things_of_last_week:
        ...
        send_mail("Weekly news", message_text, "sitestock@gmail.com",[user['email']], fail_silently=False,)


@shared_task()
def weekly_newsletter():
    all_users = list(User.objects.filter(is_staff=False).values())
    all_things_of_last_week = list(Things.objects.filter(created_at__gte=datetime.today() - timedelta(days=7)).values())
